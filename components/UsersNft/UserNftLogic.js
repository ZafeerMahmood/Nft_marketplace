...
export const getNft = async (cacheName, url, id) => {
  if (typeof window !== "undefined") {
    ...
    let str = accounts[0];
    const folder_data = await firestore
  ...
  useEffect(() => {
    ...
    async function Nftlogic() {
      if (typeof window !== "undefined") {
        ...
        setData(data);
        //getNft(data,data1)
        const folder_data_4 = await firestore
          .doc(`metamask/${cookies.user}`)
          .get()
          .then((folder_data_4) => {
    ...
    <div className="flex flex-col  w-screen overflow-x-hidden max-h-fit dark:bg-black bg-white  ">
      ...
      <div className="flex flex-col h-96 w-full bg-zinc-800">
        ...
        <div className="flex flex-row h-28 w-full px-5 justify-start rounded-full -my-32 ">
          <img
            src={alph}
            className="w-32 rounded-full border-2 dark:border-gray-600 bg-slate-900 border-gray-600 "
          />
        </div>
      </div>
      <div className="flex flex-col h-24 w-2/4  space-y-4">
        <input
          className=" h-8 w-full ml-5 dark:text-white text-gray-900 bg-transparent font-bold text-xl  "
          value={data3}
          readOnly
        />
        <input
          className=" h-8 w-full ml-5 bg-transparent text-gray-600 "
          value={cookies.user}
          readOnly
        />
      </div>
